...
void main() {
  group('Offline and Caching Integration Tests', () {
    ...
    testWidgets('Verify messages survive app restart (caching)', (tester) async {
      if (kIsWeb) return; 
      ...
      final uniqueCacheMessage = 'OFFLINE_CACHE_${DateTime.now().millisecondsSinceEpoch}';
      ...
      await fastWait($.tester, () => find.textContaining(uniqueCacheMessage).evaluate().isNotEmpty, timeout: const Duration(seconds: 30));
      ...
      await fastWait($.tester, () => $(Scrollable).exists, timeout: const Duration(seconds: 60));
      ...
      await fastWait($.tester, () {
        return find.textContaining(uniqueCacheMessage, skipOffstage: false).evaluate().isNotEmpty;
