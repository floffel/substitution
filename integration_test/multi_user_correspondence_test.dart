...
void main() {
  group('Multi-User Correspondence Integration Tests', () {
    ...
    testWidgets('UI User receives message from background SDK user', (tester) async {
      if (!await skipIfNoMatrix(matrixServer: testMatrixServer)) return;
      ...
      if (!kIsWeb) {
        ...
        final database = await openDatabase(dbPath, version: 1, onCreate: (db, v) => db.execute('CREATE TABLE clients (id TEXT PRIMARY KEY, homeserver_url TEXT, token TEXT, user_id TEXT)'));
        bgDatabase = await MatrixSdkDatabase.init('BgUser', database: database);
      } else {
      ...
      try {
        ...
        final generalRoomId = rooms1.firstWhere((id) => client1.getRoomById(id)?.name == 'test_general');
        
        // Wait for background user to see the room
        debugPrint('MULTI_USER: Waiting for background user to see room $generalRoomId...');
        await fastWait($.tester, () => client2.getRoomById(generalRoomId) != null, timeout: const Duration(seconds: 30));
        final room2 = client2.getRoomById(generalRoomId)!;

        // 4. Background User sends a message
        final uniqueBody = 'MESSAGE_FROM_BG_${DateTime.now().millisecondsSinceEpoch}';
