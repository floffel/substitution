...
void main() {
  ...
  group('Message Type Rendering Integration Tests', () {
    ...
    testWidgets(
      'Feed correctly renders text and image messages',
      (tester) async {
        if (!await skipIfNoMatrix(matrixServer: testMatrixServer)) return;
        ...
        await room.sendEvent({
          ...
          'info': {'mimetype': 'image/png', 'size': 100, 'w': 10, 'h': 10},
        });
        ...
        await fastWait($.tester, () {
          final hasText = timeline.events.any((dynamic e) {
            try {
              return e.body.toString().contains(textBody);
            } catch (_) {
              return false;
            }
          });
          final hasImage = timeline.events.any((dynamic e) {
            try {
              return e.body.toString().contains('Types Image');
            } catch (_) {
              return false;
            }
          });
        ...
        await fastWait($.tester, () {
          final hasText =
              find
                  .textContaining(textBody, skipOffstage: false)
                  .evaluate()
                  .isNotEmpty;
