...
void main() {
  group('Comment Nesting and Collapsing Integration Tests', () {
    ...
    testWidgets('Deeply nested comments render and can be navigated', (tester) async {
      if (!await skipIfNoMatrix(matrixServer: testMatrixServer)) return;
      ...
      final room = client.getRoomById(rooms.firstWhere((id) => client.getRoomById(id)?.name == 'test_general'))!;
      ...
      await fastWait($.tester, () => timeline.events.any((e) => e.eventId == rootId), timeout: const Duration(seconds: 30));

      Event lastEvent = timeline.events.firstWhere((e) => e.eventId == rootId);
      ...
      for (int i = 1; i <= nestingLevel; i++) {
        ...
        await fastWait($.tester, () => timeline.events.any((e) => e.eventId == replyId), timeout: const Duration(seconds: 20));
      ...
      await fastWait($.tester, () => $(CommentWidget).exists, timeout: const Duration(seconds: 60));
      ...
      try {
        await $.tester.scrollUntilVisible(continueButtonFinder, 500.0, scrollable: find.byType(Scrollable).first);
      } catch (e) {
        debugPrint('NESTING: scrollUntilVisible failed (maybe already visible or not present): $e');
      }
