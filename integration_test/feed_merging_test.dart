    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await fastWait(
          $.tester,
          () =>
              client.getRoomById(roomIdA) != null &&
              client.getRoomById(roomIdB) != null,
        );
        ...
        debugPrint(
          'MERGING: Room A PL for ${client.userID}: ${roomA.getPowerLevelByUserId(client.userID!)}',
        );
        debugPrint(
          'MERGING: Room B PL for ${client.userID}: ${roomB.getPowerLevelByUserId(client.userID!)}',
        );
        ...
        final eventId1 = await roomA.sendTextEvent(
          "Chronological Message 1 (A)",
        );
        await Future.delayed(const Duration(seconds: 1));

        debugPrint("MERGING: Sending Message 2 in Room B...");
        final eventId2 = await roomB.sendTextEvent(
          "Chronological Message 2 (B)",
        );
        await Future.delayed(const Duration(seconds: 1));

        debugPrint("MERGING: Sending Message 3 in Room A...");
        final eventId3 = await roomA.sendTextEvent(
          "Chronological Message 3 (A)",
        );
        ...
        await fastWait($.tester, () {
          ...
          if (anyA || anyB) {
            debugPrint(
              'MERGING: Found some messages. has1=$has1, has2=$has2, has3=$has3',
            );
          }
        ...
        for (int i = 0; i < chronoFinders.length - 1; i++) {
          ...
          expect(
            posTop.dy,
            lessThan(posBottom.dy),
            reason: "Message $i should be above Message ${i + 1}",
          );
        }
