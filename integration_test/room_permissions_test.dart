...
void main() {
  group('Room Permissions Integration Tests', () {
    ...
    testWidgets('Admin can navigate to permissions and toggle blog mode', (tester) async {
      if (!await skipIfNoMatrix(matrixServer: testMatrixServer)) return;
      ...
      await fastWait($.tester, () => find.textContaining('test_general').evaluate().isNotEmpty);
      ...
      final roomTile = find.ancestor(of: find.textContaining('test_general'), matching: find.byType(RoomWidget));
      final settingsButton = find.descendant(of: roomTile, matching: find.byIcon(Icons.settings));
      ...
      await fastWait($.tester, () => $(RoomPermissionsPage).exists, timeout: const Duration(seconds: 30));
      ...
      debugPrint('✓ PERMISSIONS: Successfully toggled and verified mode change');
