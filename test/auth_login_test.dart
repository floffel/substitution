...
void main() {
  ...
  group('Login Page - Additional Behavior Tests', () {
    ...
    testWidgets(
      '7a. SSO-only server: shows SSO buttons, hides password fields',
    ...
    testWidgets(
      '7b. Server supporting both password and SSO shows all buttons',
      (WidgetTester tester) async {
        final mockClient = MockClient();
        when(() => mockClient.isLogged()).thenReturn(false);
        when(
  ...
  group('Real-world server login flow fixtures', () {
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        expect(
          find.byKey(const Key('ssoButton_oidc-codeberg')),
          findsOneWidget,
        );
